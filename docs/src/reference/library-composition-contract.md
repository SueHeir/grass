...
- A library **MUST** own its simulation state as typed App resources; a system
  **MUST** declare each resource it reads as `Res<T>` and each it mutates as
  ...
  state. [Scheduler access tests](https://github.com/SueHeir/grass/blob/main/crates/grass_scheduler/src/lib.rs#L255-L301)
  exercise the declared access inventory.
...
- A composed app **MUST** give independently authored phase enums distinct
  ...
  cover the installed-schedule path.
...
- An external driver that uses `prepare()`/`run()` rather than `start()`
  **MUST** call `run_cleanup()`; a parent that owns sub-Apps **MUST** call
  ...
  automatic transitive cleanup. [App lifecycle implementation](https://github.com/SueHeir/grass/blob/main/crates/grass_app/src/app.rs#L454-L484)
  and the [coupling tutorial](../tutorial/coupling-two-solvers.md#8-run-it)
  document both paths.
...
- A substitutable service **MUST** be expressed as an exported `CapabilityId`;
  ...
  **MUST** use `try_prepare`, `try_start`, or
  ...
  path. [Capability tests](https://github.com/SueHeir/grass/blob/main/crates/grass_app/src/app.rs#L1216-L1330)
...
- A library that mirrors a resource across host and device **MUST** register a
  ...
  scheduler's lazy download. [Coherence tests](https://github.com/SueHeir/grass/blob/main/crates/grass_scheduler/src/coherence.rs#L296-L358)
...
- Grass execution is currently single-threaded and deterministic: phase/
  ...
  guarantee from this. [Ordering tests](https://github.com/SueHeir/grass/blob/main/crates/grass_scheduler/src/lib.rs#L3787-L4030)
  exercise the observable trace.
