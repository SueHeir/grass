...
| [`Plugin`](src/plugin.rs) | trait whose `build(&self, app: &mut App)` wires up a feature. Optional hooks include typed capability contracts and `config_description` (typed TOML metadata; `default_config` remains for legacy snippets). A bare `Fn(&mut App)` closure also implements `Plugin` |
...
| [`ConfigSnippets`](src/app.rs) / [`GenerateConfigFlag`](src/app.rs) | plugins emit generated TOML from `config_description` (or a legacy `default_config` fragment); adding `GenerateConfigFlag` makes `start()` print the assembled config and exit |
