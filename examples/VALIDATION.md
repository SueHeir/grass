...
- [`oscillator_mpmd`](oscillator_mpmd/README.md) embeds
  ...
  comparing the complete in-process `LocalTransport` trajectory of the
  two-binary exchange contract with an independently implemented explicit
  recurrence.  Its CI MPMD launch records both binary traces and compares the
  full MPI trajectory with that same recurrence; bit fingerprints have a
  documented heterogeneous-floating-point warning policy.
