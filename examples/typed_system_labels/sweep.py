#!/usr/bin/env python3
"""Run and graph the typed scheduler-label validation matrix."""
...
def main() -> None:
    ...
    if len(passed) != len(rows):
        raise SystemExit(f"FAIL: {len(passed)}/{len(rows)} scheduler-label checks matched")

    out = HERE / "plots"
    out.mkdir(exist_ok=True)
    canvas = Canvas(900, 410)
    canvas.text(30, 25, "TYPED SYSTEM LABEL VALIDATION", BLACK, 3)
    canvas.text(30, 52, "observed result compared with declarative expected result", BLUE, 2)
    ...
    for index, (case, wanted, actual, _) in enumerate(rows):
        ...
        canvas.rect(x, 145, x + 115, bottom, color)
        canvas.text(x + 28, 118, "MATCH" if wanted == actual else "MISMATCH", color, 2)
    ...
    canvas.text(30, 395, f"PASS: {len(passed)}/{len(rows)} required and optional label checks match", BLACK, 2)
    canvas.save(out / "typed_system_labels_matrix.png")
